#ifndef TRAJCOMP_GEOHASH_INC
#define TRAJCOMP_GEOHASH_INC

/*
 * This is mainly the geohash implementation from googlecode
 * For license and all this stuff, look there
 * 
 * However, changed the argument types from char * to const char *
 * in order not to copy strings.
 * 
 * Our interface class ends this file.
 * */
 ...
 */
static int geohashstr_to_interleaved(const char* r, size_t length, uint16_t *interleaved, size_t dst_count){
...
*/
static int geohash_decode_impl(const char* r, size_t length, double *latitude, double *longitude){
...
int geohash_decode(const char* r, size_t length, double *latitude, double *longitude){
...
{
	public:
	std::string operator() (double lat, double lon, size_t geohash_length = 5)
	{
		int res;
		char r[128];
		if((res=geohash_encode(lat,lon, r, geohash_length+1))==GEOHASH_OK){
			return std::string(r);
		}else{
			std::stringstream ss;
			ss << "Unable to geohash:" << lat << " " << lon;
			throw std::runtime_error(ss.str());
		}
	}
	
	void decode(std::string s, double &lat, double &lon)
	{
//		int geohash_decode(char* r, size_t length, double *latitude, double *longitude);
		int res;
		if((res=geohash_decode(s.c_str(),s.size(), &lat, &lon))!=GEOHASH_OK)
		{
			std::stringstream ss;
			ss << "Unable to decode geohash:" << s;
			throw std::runtime_error(ss.str());
		}
...
class op_ghvector
{
	size_t geohash_length;
	public:
		op_ghvector(size_t gl):geohash_length(gl)
		{
			//cout << "Operating using " << geohash_length << endl;
		}
		std::string operator ()(ElementType &elt)
		{
			static trajcomp::geohash gh;	
			std::string s;
			try{
			s = gh(elt[0],elt[1],geohash_length);
			}catch(...)
			{
			s = "ERROR";	
			}
...
std::vector<std::string> trajectory2geohashsequence(Trajectory &t, size_t geohash_length)
{
	std::vector<std::string> ret;
	ret.resize(t.size());
	std::transform(t.begin(),t.end(),ret.begin(),op_ghvector<typename Trajectory::value_type>(geohash_length));
	return ret;
}

template<class Trajectory>
std::vector<std::string> trajectory2geohashset(Trajectory &t,size_t geohash_length)
{
	std::vector<std::string> ret = trajectory2geohashsequence(t,geohash_length);
